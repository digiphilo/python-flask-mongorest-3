...
class Resource(object, metaclass=ResourceMeta):
    ...
    def get_rename_fields(self):
        ...
        """
        rename_fields = {}
        for k, v in list(self.rename_fields.items()):
            rename_fields[k] = v
            if v not in self.rename_fields:
                rename_fields[v] = k
    ...
    def get_objects(self):
        ...
        for key in params:
            ...
            if op_name not in list(allowed_operators.keys()):
                continue
        ...
        try:
            ...
            def serialize_errors(errors):
                if hasattr(errors, 'items'):
                    return {k: serialize_errors(v) for k, v in list(errors.items())}
                else:
                    return str(errors)
            raise ValidationError({'field-errors': 'found'})

    def create_object(self, data=None, save=True, parent_resources=None):
        kwargs = {}
        data = data or self.data
        for field in self.get_fields():
            if field in list(self.document._fields.keys()) and field not in self.readonly_fields and (type(data) is list or (type(data) is dict and field in data)):
    ...
    def update_object(self, obj, data=None, save=True, parent_resources=None):
        data = data or self.data
        for field in self.get_fields():
            if field in list(self.document._fields.keys()) and field not in self.readonly_fields and field in data:
